...
export async function buoyRoutes(app: FastifyInstance) {
  ...
  app.get<{ Querystring: { lat: string; lon: string; facing?: string } }>(
    ...
    async (request, reply) => {
      ...
      const facing =
        request.query.facing !== undefined
          ? parseFloat(request.query.facing)
          : undefined;

      if (isNaN(lat) || isNaN(lon)) {
        return reply
          .status(400)
          .send({ error: "lat and lon must be valid numbers" });
      }
      ...
      if (!conditions) {
        return reply
          .status(404)
          .send({ error: "No buoy data available for this location" });
      }
